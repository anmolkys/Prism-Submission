...
class GmailTasks():
  ...
  def action_required_emails_task(agent):
    ...
    return Task(
      ...
      If applicable schedule events in the calender using `schedule_events_task` agent. 
  ...
  def draft_response_task(agent):
    ...
    return Task(
      ...
      -to (sender's email address to be responded)
      -subject
      -message
      You MUST create all drafts before sending your final answer. 
      Your final answer MUST be a confirmation that all responses have been drafted. Use Nss as signature.
